...
// Suggestion review by CodeRabbit
...
class SuggestionWidget extends WidgetType {
  ...
  toDOM() {
    ...
    span.style.pointerEvents = "none"; // Don't interface with clicks
    return span;
  }
}
...
const renderPlugin = ViewPlugin.fromClass(
  class {
    ...
    update(update: ViewUpdate) {
      ...
      const suggestionChange = update.transactions.some((transaction) =>
        transaction.effects.some((effect) => effect.is(setSuggestionEffect)),
      );

      // Rebuild decorations if doc changed, cursor moved, or suggestion changed
      const shouldRebuild =
        update.docChanged || update.selectionSet || suggestionChange;
    ...
    build(view: EditorView) {
      ...
      return Decoration.set([
        Decoration.widget({
          ...
          side: 1, // Render after cursor (sid: 1), not before (side: -1)
        }).range(cursor),
      ]);
    }
  },
...
const acceptSuggestionKeymap = keymap.of([
  {
    ...
    run: (view) => {
      ...
      view.dispatch({
        ...
        selection: { anchor: cursor + suggestion.length }, // Move cursor to end Line
